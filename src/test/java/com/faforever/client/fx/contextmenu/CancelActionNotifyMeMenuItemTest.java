...
public class CancelActionNotifyMeMenuItemTest extends UITest {
  ...
  public void testOnClickedCancelActionNotifyMe() {
    instance.onClicked();
    Mockito.verify(liveReplayService).stopTrackingReplay();
  }

  @Test
  public void testVisibleItem() {
    GameBean game = GameBeanBuilder.create().defaultValues().get();
    Mockito.when(liveReplayService.getTrackingReplay()).thenReturn(Optional.of(new Pair<>(game.getId(), LiveReplayAction.NOTIFY_ME)));
