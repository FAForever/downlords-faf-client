...
@Slf4j
public abstract class AbstractCheckForUpdateTask extends CompletableTask<Optional<UpdateInfo>> {
  ...
  @Override
  protected final Optional<UpdateInfo> call() throws Exception {
  ...
  abstract protected Optional<UpdateInfo> getUpdateInfo(ClientConfiguration clientConfiguration);
  ...
  protected Optional<Configuration> getCurrentConfiguration(List<GitHubRelease> responseBody) {
    return responseBody.stream()
        .filter(gitHubRelease -> versionFromTag(gitHubRelease.getTagName()).equals(Version.getCurrentVersion()))
        .findFirst()
        .flatMap(this::getUpdate4jConfigAsset)
        .map(GitHubAsset::getBrowserDownloadUrl)
        .map(this::readConfiguration)
        .or(this::generateCurrentConfiguration);
  }

  private Optional<Configuration> generateCurrentConfiguration() {
    String appDir = System.getProperty("install4j.appDir");
    if (appDir == null) {
      return Optional.empty();
    }
    return Optional.ofNullable(Configuration.builder()
        .baseUri("http://localhost")
        .basePath(appDir)
        .files(FileMetadata.streamDirectory(Paths.get(appDir))).build());
  }

  protected Optional<GitHubAsset> getUpdate4jConfigAsset(GitHubRelease latestRelease) {
    return latestRelease.getAssets().stream()
        .filter(this::matchesPlatformUpdate4jXml)
        .findFirst();
  }

  private boolean matchesPlatformUpdate4jXml(GitHubAsset asset) {
    if (Platform.isWindows()) {
      return asset.getName().equals("update4j-win.xml");
    }
    if (Platform.isLinux()) {
      return asset.getName().equals("update4j-unix.xml");
    }
    return asset.getName().equals("update4j-mac.xml");
  }
  ...
  protected ComparableVersion versionFromTag(String tagName) {
    return new ComparableVersion(tagName.replaceAll("^v", ""));
  }
}
